...
### MOMENTO 1: Navegación Interna — El Teletransportador
**Tiempo:** 30 min

1. **Concepto de navegación** — qué significa moverse en la web (navegación externa vs. interna)
2. **La etiqueta `<a>`** — el enlace: anatomía de `href`, texto visible y cierre
3. **El atributo `id`** — el DNI del elemento: reglas, unicidad, sin espacios
4. **El enlace ancla `href="#"`** — conectando el menú con las secciones (`#` + `id`)
5. **Code-along completo (Lab 02 — Parte 3):** ids en secciones → enlaces en `<nav>` → prueba en navegador

<aside>

**IDEAS-NOTAS-TAREAS:**

- Este momento es el cierre del Lab 02 (Parte 3) que no se alcanzó en la Clase 02
- Usar el canvas de Excalidraw ya preparado: diagrama de conexión `href="#id"` → `section id`
- El code-along construye sobre el HTML semántico que los alumnos ya tienen
- Checkpoint final: screenshot con los enlaces del `<nav>` visibles en el navegador
</aside>
...
### 1.1 Concepto de navegación — qué significa moverse en la web
...
### 1.3 El atributo `id` — El DNI del elemento
...
> **Tu explicación teórica precisa:**
> *"El atributo `id` es el DNI de un elemento HTML. Así como cada persona tiene un número de identificación único — ninguna persona tiene el mismo — cada elemento con `id` tiene un nombre irrepetible en la página.*
>
...
### 1.4 El enlace ancla `href="#"` — Conectando el menú con las secciones
...
### 1.5 Lab 02 — Parte 3: Code-along completo (navegación funcional)
...
> *"Primero, vamos a las secciones dentro del `<main>` y les agregamos su `id`. Usen el nombre del tema que pusieron en el wireframe — esos nombres ya los pensaron en la clase anterior."*
...
> *"Ahora subimos al `<nav>` que dejamos vacío en la clase anterior. Aquí ponemos un `<a>` por cada sección — usando el `#` más el `id` exacto que acabamos de escribir."*
...
> **Checkpoint 1.5 — Screenshot final:**
...
> El Lab 02 está cerrado con navegación funcionando. A partir de aquí comienza el contenido propio de la Clase 03. Cambiar el tono — lo que viene no es deuda, es terreno nuevo: CSS. 

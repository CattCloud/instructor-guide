...
panelPlaceholder(ox, '1.2', 'MOMENTO 1 - IMAGEN', 'IMG-01', 'Persistencia (conceptual/real)', 1200, 900); ox += 1200 + GAP;
panelPlaceholder(ox, '1.3', 'MOMENTO 1 - IMAGEN', 'IMG-02', 'Dentro de localStorage', 1200, 900); ox += 1200 + GAP;
panelPlaceholder(ox, '1.4', 'MOMENTO 1 - IMAGEN', 'IMG-03', 'JSON como traductor', 1200, 900); ox += 1200 + GAP;

{ const y = header(ox, '1.5', 'MOMENTO 1', 'Desarrollar la HU1 (LIBRE)', PAL.verde);
  marcoLibre(ox, y, LW, LH, 'HU1 - GUARDAR LAS PLANTILLAS',
    '"Como usuario, quiero que mis plantillas se\nguarden automaticamente, para no perderlas\nal recargar."',
    '1. Al agregar/editar/eliminar, se guarda.\n2. Guardado automatico (sin boton).\n3. Se comprueba en DevTools.', 3); ox += LW + GAP; }

panelPlaceholder(ox, '2.2', 'MOMENTO 2 - IMAGEN', 'IMG-04', 'Deserializacion (flujo)', 1300, 800); ox += 1300 + GAP;

{ const y = header(ox, '2.3', 'MOMENTO 2', 'Desarrollar la HU2 (LIBRE)', PAL.naranja);
  marcoLibre(ox, y, LW, LH, 'HU2 - CARGAR AL ABRIR',
    '"Como usuario, quiero ver mis plantillas\nguardadas apenas abro la app."',
    '1. Al abrir, aparecen las guardadas.\n2. Si no hay nada, vacia sin errores.\n3. Las fechas se muestran bien.', 3); ox += LW + GAP; }

{ const y = header(ox, '3.2', 'MOMENTO 3', 'Desarrollar la HU3 (LIBRE)', PAL.rojo);
  marcoLibre(ox, y, LW, LH, 'HU3 - NO ROMPER CON DATOS CORRUPTOS',
    '"Como usuario, quiero que la app funcione\naunque los datos guardados esten dañados."',
    '1. Si el guardado esta corrupto, no se cuelga.\n2. Arranca vacia y usable.', 2); ox += LW + GAP; }

{ const y = header(ox, '4.2', 'MOMENTO 4', 'Desarrollar la HU4 (LIBRE)', PAL.verde);
  marcoLibre(ox, y, LW, LH, 'HU4 - VACIAR TODO + INDICADOR',
    '"Como usuario, quiero borrar todas mis\nplantillas de golpe y saber que la app guardo."',
    '1. Accion para vaciar todas.\n2. Lista y almacenamiento limpios.\n3. Indicador de estado.', 3); ox += LW + GAP; }

{ const y = header(ox, '5.2', 'MOMENTO 5', 'Desarrollar la HU5 (LIBRE)', PAL.azul);
  marcoLibre(ox, y, LW, LH, 'HU5 - RECORDAR EL FILTRO',
    '"Como usuario, quiero que el filtro que\nescribi siga aplicado si recargo."',
    '1. Al recargar, el filtro sigue aplicado y\n   el buscador muestra el texto.\n2. Si no habia, arranca vacio.', 3); ox += LW + GAP; }
...
const outPath = process.argv[2] || 'CLASE 15.excalidraw';
